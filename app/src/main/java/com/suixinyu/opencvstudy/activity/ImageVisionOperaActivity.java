...
//        Imgproc.dilate(mat, dst, matRect);
     ...
     * 高斯模糊/滤波：权值计算原矩阵和核矩阵的元素和 中心点的权值比较高 两边点的权值比较低
     * 和均值不同不需要求平均值直接求权值和
     * api(GaussianBlur):源mat/目标mat/卷积核矩阵/x轴模糊系数/y轴模糊系数/模糊边缘类型
     * 卷积核矩阵可以不同但是必须保持正数和奇数
     * 设置x轴模糊系数不设置y轴模糊系数 系统会自动计算y轴模糊系数 xy相等
     * 设置x轴模糊系数可以不设置size 会根据x轴y轴系数自动计算size 公式：sigmax = 0时 sigma = 0.3*（（ksize-1）*0.5-1）+ 0.8
